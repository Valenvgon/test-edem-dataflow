...
def ParsePubSubMessage(message):
    ...
    return message["vehicle_id"], message

 #   logging.info("new message in pub sub: %s", message)
...
def getTrafficImage(item, api_url):
    ...
    image_bytes = json.loads( image_bytes.decode('utf-8') )['image_url']
...
class FormatFirestoreDocument(beam.DoFn):
    ...
    def process(self, element):
        ...
        #Lista de elementos
        events = ["battery_info", "driving_info", "environment_info"] if self.mode == "raw" else ["battery_info, driving_info, environment_info, autonomy_info, inspection"]
        ...
        for event in events:
            ...
            if len(element[event]) > 0:
                ...
                try:
                    db.collection("vehicle_telemetry_data").document(element["vehicle_id"]).collection(event).document(element[event]["timestamp"]).set(element[event])
                    logging.info("Document written to Firestore: %s", element)
                except Exception as err:
                    logging.error("Error writing document to Firestore: %s", err)
...
class CalculateAutonomyDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        if is_valid:
            ...
            dict['autonomy'] = autonomy
            dict['efficiency'] = efficiency
            dict['traffic_level'] = traffic_level
    ...
    with beam.Pipeline(argv=pipeline_opts, options=options) as p:
        ...
        for telemetry_type, subscription in telemetry_sources.items():
            telemetry_data[telemetry_type] = (
                ...
                | f"Fixed Window for {telemetry_type.capitalize()} Telemetry Data" >> beam.WindowInto(window.FixedWindows(60))
            )

        # CoGroupByKey
        grouped_data = (
            telemetry_data["battery"], telemetry_data["driving"], telemetry_data["environment"]
        ...
        (
            ...
            | "Write non_critical_battery_users documents" >> beam.ParDo(FormatFirestoreDocument(mode='raw'))
        )
        ...
        send_data = (
            ...
            | "Calculate Autonomy" >> beam.ParDo(CalculateAutonomyDoFn())
        )

        (
            send_data
            | "Encode notifications" >> beam.Map(json.dumps)
            | "Write notifications to PubSub" >> beam.io.WriteToPubsub(topic=args.output_topic)
        )

        (
            send_data
            | "Write critical_battery_users documents" >> beam.ParDo(FormatFirestoreDocument(mode='critical'))
        )
