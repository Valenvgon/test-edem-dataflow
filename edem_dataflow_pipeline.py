...
def ParsePubSubMessage(message):
    ...
    pubsub_message = message.decode('utf-8')  #decodeamos 
    
    # Convert string decoded in JSON format
    msg = json.loads(pubsub_message) #leemos el json

    logging.info("New message in PubSub: %s", msg) #pintamos esto para ver los logs por cajita en gcp y nos lo preparamos ya que luego se utiliza de nuevo, el %s, msg lo que hacemos es llamar a msg y que %s sea msg

    # Return function
    return msg['vehicle_id'], msg  #dentro del msg sacamos el id del vehivulo y luego cargamos todo el msg para no perder ningun dato ya que lo hacemos en modo clave valor 
        # el msg es el mensaje directo entero de pubsub 
...
class FormatFirestoreDocument(beam.DoFn):
    ...
    def process(self, element):
        ...
        events = ['battery_info', 'driving_info', 'environment_info'] if self.mode == 'raw' else ['battery_info', 'driving_info', 'environment_info', 'inspection'] #los raw seran los mayores de 30 y los proces seran los menores de 30
...
class BusinessLogicDoFn(beam.DoFn): #lo hacemos en Dofn ya que las transformaciones son complejas son de dos o mas pcoll de salida y paraleliza gran cantidada de datps 
    ...
    def process(self, element):
        ...
        vehicle_id, (battery_data, event_data, environment_data) = element       #a, (x,y,z) = b
        ...
        if battery_info and battery_info.get("battery_level", 100) < 30 and battery_info.get("event_type") != "charging":  ## primera condicion es que battery info exista despues sacar que el level battery sea menor a treinta y que no este cargando
            
            yield beam.pvalue.TaggedOutput("critical_battery_users", vehicle_telemetry_data) # como son mas de 1 salida lo hacemos con yield 
            # esto es vehicle_telemetry_data es el element es decir el output el critical_battery_users es un nombre que espera tal como una función
        
        else:
            yield beam.pvalue.TaggedOutput("non_critical_battery_users", vehicle_telemetry_data)
            # igual pero los sanos 
...
class CalculateAutonomyDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        if is_valid:
            ...
            yield dict  #el yield es porque estamos usando dofn
...
def run():
    ...
    ## sera donde la google function estará escuchando
    ...
    with beam.Pipeline(argv=pipeline_opts,options=options) as p: #quiero que el pipeline tenga unas opciones y le ponemos un alias 
        ...
        battery_data = (
            p 
                | "Read Battery Telemetry Data From PubSub" >> beam.io.ReadFromPubSub(subscription=args.battery_telemetry_subscription)  #transformacion de tipo input que nos deja leer y por abajo hace la llamada para leer a pubsub 
                | "Parse JSON battery messages" >> beam.Map(ParsePubSubMessage) #utilizamos map ya que es una transformacion io uno a uno 
                    #creamos un metodo que es PArsePubMessage, no le ponemos ningun argumento por que viene de arriba de la propia función
                | "Fixed Window for Battery Telemetry Data" >> beam.WindowInto(beam.window.FixedWindows(60)) #hcemos la ventana de 60 segundos 
                    # ver mejor en las diapositivas el tipo de ventanas y las diferencias entre ellas
        )
        ...
        environment_data = (
            ...
            # creamos tres picollections diferentes para leer desde pubsub y hgaciendo una ventana de 60
        

        # CoGroupByKey
        grouped_data = (
            battery_data, driving_data, environment_data) | "Merge PCollections" >> beam.CoGroupByKey() # de las tres pcollections de la izquierda generaré la nueva pcollection cpn Cogroupbykey
        
        grouped_data | "print pcoll" >> beam.Map(print)
        
        processed_data = (grouped_data
            | "Check battery level" >> beam.ParDo(BusinessLogicDoFn()).with_outputs("critical_battery_users", "non_critical_battery_users"))  #aqui es donde hacemos 30 o menos de 30 pardo simepre va con Dofn



# los que sean de ,ayores de 30 se van a firestore:
        (
        ...
        ) #con esto los que no son críticos se van a firestore 

        send_data = (   #ahora estamos en el punto de que ya hemos terminado con los >30 y vamos a por los menores de 30
        ...
        (
            send_data
                ...
                | "Write notifications to PubSub" >> beam.io.WriteToPubSub(topic=args.output_topic) # esta se ira a pub/sub 
        )
